...
public class TileEntityMagneticInductionFurnace extends TileEntityMagnetic implements ISidedInventory {
    ...
    public void updateEntity()
    {
        ...
        if (!this.worldObj.isRemote)
        {
			...
			if(cooldown <= 0)
			{
				if (this.canSmelt() && gauss > cap(1000*getItemRarity()))
				{
					gauss -= cap(1000*getItemRarity());
					System.out.println(cap(1000*getItemRarity()));
					this.smeltItem();
					flag1 = true;
					cooldown = 1;
				}
	...
	private float cap(int i) {
		return i<1000?1000:i<72000?i:72000;
	}
	@Override
	public void dieOff() 
	{
	...
	private int getItemRarity() {
		ItemStack target = this.furnaceItemStacks[0];
		if(WorldgenMonitor.multipliers.containsKey(this.worldObj.getChunkFromBlockCoords(xCoord, zCoord).getBiomeArray()[0]))
		{
			if(WorldgenMonitor.multipliers.get(this.worldObj.getChunkFromBlockCoords(xCoord, zCoord).getBiomeArray()[0]).containsKey(target.itemID))
			{
				return WorldgenMonitor.multipliers.get(this.worldObj.getChunkFromBlockCoords(xCoord, zCoord).getBiomeArray()[0]).get(target.itemID);
			}
		}
		return -1;
	}
